...
class NewsModel: GBBaseModel {
    ...
    var collectionViewH: CGFloat {
        return Calculate.collectionViewHeight(images.count)
    }
    /// collectionView 宽度
    var collectionViewW: CGFloat {
        return Calculate.collectionViewWidth(images.count)
    }
    ...
    var id:String?
    var usefulCnt:Int = 0
    var usefulChecked = false
    var shared = false
    var watched = false


    var attributedContent: NSMutableAttributedString { return emojiManager.showEmoji(content: content!, font: XKFont.pingFangSCRegular.size(14)!)}
    var titleHeight: CGFloat { return title.textHeight(fontSize: 16, width: kScreenWidth - 30) }
    var contentH: CGFloat { return content!.textHeight(fontSize: 14, width: kScreenWidth - 30) }
    var item_id: Int = 0
    var cell_flag: Int = 0
