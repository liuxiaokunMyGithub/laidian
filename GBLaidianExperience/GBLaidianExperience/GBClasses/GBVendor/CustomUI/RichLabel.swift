...
class RichLabel: UILabel {
    ...
    override var attributedText: NSAttributedString? {
        didSet {
            ...
            _ = userRanges.map { textStorage.addAttribute(.foregroundColor, value: UIColor(r: 72, g: 100, b: 149), range: $0) }
            // 匹配话题
            topicRanges = ranges(from: "#.*?#")
            _ = topicRanges.map { textStorage.addAttribute(.foregroundColor, value: UIColor(r: 72, g: 100, b: 149), range: $0) }
            // 匹配链接
            linkRanges = rangesOfLink()
            _ = linkRanges.map { textStorage.addAttribute(.foregroundColor, value: UIColor(r: 72, g: 100, b: 149), range: $0) }
        }
    }
