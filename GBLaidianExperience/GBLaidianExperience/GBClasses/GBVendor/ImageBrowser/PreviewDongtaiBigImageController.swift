...
//        SVProgressHUD.configuration()
        ...
        collectionView.register(cellType: DongtaiCollectionViewCell.self)
    ...
    @IBAction func saveButtonClicked(_ sender: UIButton) {
        ...
        ImageDownloader.default.downloadImage(with: URL(string: BASE_URL_IMAGE_QINIUYUN + largeImage)!, progressBlock: { (receivedSize, totalSize) in
            // 获取当前进度
...
extension PreviewDongtaiBigImageController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(for: indexPath, cellType: DongtaiCollectionViewCell.self) as DongtaiCollectionViewCell
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        selectedIndex = Int(scrollView.contentOffset.x / kScreenWidth + 0.5)
        indexLabel.text = "\(selectedIndex + 1)/\(images.count)"
    }
}
