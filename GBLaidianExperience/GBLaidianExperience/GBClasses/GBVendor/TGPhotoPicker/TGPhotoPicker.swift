...
class TGPhotoPicker: UIView {
    ...
    var onClickFinishButton:(([TGPhotoM])->())?
 ...
 */
    ...
    init(_ vc: UIViewController, frame: CGRect,_ configBlock:((_ config:TGPhotoPickerConfig)->())? = nil) {
        ...
        if self.config.autoSelectWH {
            ...
            self.config.mainCellWH = (frame.size.width -
                (self.config.mainColCount+(self.config.leftAndRigthNoPadding ? -1 : 1))
                * self.config.padding-0.1) / self.config.mainColCount
        }
    ...
    fileprivate func addPhotos(){
        TGPhotoPickerManager.shared.authorizePhotoLibrary { (status) in
            if status == .authorized{
                ...
                for model in self.tgphotos {
//                    if !self.selectImages.contains(model.imageData!) {
                        dataArray.append(model.asset!)
//                    }
                }
                pickervc.assetArr = dataArray
        ...
        case 0:
            if TGPhotoPickerConfig.shared.useiOS8Camera{
                ...
                cameraVC.callbackPicutureData = { [weak self] imgData in
                    ...
                    let imgData =
                        bigImg?.jpegData(compressionQuality: TGPhotoPickerConfig.shared.compressionQuality)
                    let smallImg = bigImg
                    let model = TGPhotoM()
...
extension TGPhotoPicker: TGPhotoPickerDelegate{
    func onImageSelectFinished(images: [PHAsset]) {
        TGPhotoM.getImagesAndDatas(photos: images) { array in
            self.tgphotos = array
            self.onClickFinishButton!(self.tgphotos)
            DispatchQueue.main.async {
                self.collectionView?.reloadData()
            }
        }
