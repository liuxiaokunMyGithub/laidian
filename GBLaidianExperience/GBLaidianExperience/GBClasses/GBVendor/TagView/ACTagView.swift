...
open class ACTagView: UIView {
  ...
  open weak var tagDelegate: ACTagViewDelegate?
    open var selectMaxCount: Int = 0
    fileprivate var selectCount: Int = 0

  open var tagWidth: CGFloat = 0
...
extension ACTagView: UICollectionViewDelegateFlowLayout {
  ...
  public func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
    ...
    var width = tagWidth > 0 ? tagWidth : tagAttribute.getWidth(height: tagHeight)
    if width < tagHeight {
      width = tagHeight
    }
  ...
  public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
    ...
    if selectMaxCount != 0 {
        if selectCount == selectMaxCount {
            GBToolShowMessage("最多选择\(selectMaxCount)项")
            // 需要配置cell不选中状态
